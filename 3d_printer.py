...
def optimize_printing(print_jobs: List[Dict], constraints: Dict) -> Dict:
    ...
    while print_jobs:
        ...
        for job in print_jobs[:]:
            # якщо завдання вміщується в обмеження - додаємо його до поточної групи
            if (
                ...
                # Видалення вибраних моделей із загальної черги
                print_jobs.remove(job)
        if current_group:
            # Розрахунок часу друку групи
            current_group_time = max(job.print_time for job in current_group)
            # Додавання групи до порядку друку
